...
def fileup():
    ...
    if request.method == 'POST':
        try:
            ...
            accuracy = accs['VotingClassifier']  # Show Voting accuracy on page
            ...
            message = "Model trained and chart saved!"
...
def predict():
    ...
    if request.method == 'POST':
        try:
            ...
            db = pymysql.connect(host="localhost", user="root", password="Ashika@12510",
                                 database="project_db", cursorclass=pymysql.cursors.DictCursor)
            cursor = db.cursor()
            user_id = session.get('user_id', 1)
            email= session.get('email','exg@gmail.com')
            for model_name, pred in preds.items():
                cursor.execute("""
                    INSERT INTO manual_predictions (user_id, email, model_name, input_data, prediction_result, actual_result)
                    VALUES (%s, %s, %s, %s, %s)
...
def chart():
    ...
    try:
        ...
        # 🎯 Chart 1: User-specific VotingClassifier Risk Distribution
        cursor.execute("""
            SELECT prediction_result 
            FROM manual_predictions 
            WHERE user_id = %s AND model_name = 'VotingClassifier'
        """, (user_id,))
        results = cursor.fetchall()

        count_0 = sum(1 for r in results if r['prediction_result'] == 0)
        count_1 = sum(1 for r in results if r['prediction_result'] == 1)

        plt.figure()
        plt.bar(['Low Risk', 'High Risk'], [count_0, count_1], color=['green', 'red'])
        plt.title("Your Risk Prediction (VotingClassifier)")
        plt.ylabel("Count")
        plt.savefig('static/graphs/risk_chart.png')
        plt.close()
...
def my_predictions():
    ...
    cursor = db.cursor()
    cursor.execute("SELECT * FROM manual_predictions WHERE user_id = %s ORDER BY created_at DESC", (session['user_id'],))
